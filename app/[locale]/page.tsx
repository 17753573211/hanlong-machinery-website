...
const homeProductImages: Record<string, string> = {
  "HL-2": "/images/products/hl-2.jpg",
  "HL-2.6": "/images/products/hl-2.6.jpg",
  "HL-4": "/images/products/hl-4.jpg",
};
...
export default async function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main>
    ...
    <section className="section machines-section"><div className="container section-top"><div><p className="eyebrow"><span/> {h.productRange}</p><h2>{h.machinesTitle}</h2></div><Link href={`/${locale}/products`} className="text-link">{d.common.viewAll}<ArrowIcon/></Link></div><div className="container product-grid">{products.filter(p=>p.slug!=="hl-35").slice(2,5).map((p,i)=><article className="product-card" key={p.model}><div className="card-top"><span>{p.capacity}</span><small>0{i+1}</small></div><div className="machine-silhouette"><Image src={homeProductImages[p.model]} alt={`${p.model} Self Loading Concrete Mixer`} fill sizes="(max-width: 680px) 92vw, (max-width: 1000px) 46vw, 30vw" style={{objectFit:"contain",objectPosition:"center"}}/></div><div className="product-meta"><div><p>{p.capacity}</p><h3>{p.model}</h3></div><Link href={`/${locale}/products/${p.slug}`}><ArrowIcon/></Link></div></article>)}</div></section>
    ...
    <section className="section home-video"><div className="container"><div className="section-top"><div><p className="eyebrow dark"><span/> {h.videoLabel}</p><h2>{h.videoTitle}</h2></div><Link href={`/${locale}/videos`} className="text-link">{d.common.viewAll}<ArrowIcon/></Link></div><video controls muted preload="metadata" poster={homeVideo.poster} playsInline><source src={homeVideo.publicPath} type="video/mp4"/></video></div></section>
