...
export default async function ProductLayout({children,params}:{children:React.ReactNode;params:Promise<{locale:string}>}){
  ...
  const product={"@context":"https://schema.org","@type":"Product","@id":`${url}#product`,name:`HL-3.5 ${d.detail.eyebrow}`,sku:"HL-3.5",category:"Self Loading Concrete Mixer",description:d.detail.description,url,image:[`${SITE_URL}${p.image}`],brand:{"@type":"Brand",name:"IRONMIX"},manufacturer:{"@id":`${SITE_URL}/#organization`},additionalProperty:p.specs.map(s=>({"@type":"PropertyValue",name:s.label,value:s.value}))};
